class Util {
    ...
    // 引用
    static ParseNames(names, offset, web3) {
        var nameList = [];
        var endIndex;
        var name;
    ...
    static ParseTimeFormat(millisecond, signal = "T") {
    ...
    static Local2Zero(millisecond) {
        var offset = new Date().getTimezoneOffset() * 60;
        return millisecond + offset;
    }
    static Zero2Local(millisecond) {
        var offset = new Date().getTimezoneOffset() * 60;
        return millisecond - offset;
    }
}
...
class Connect {
    ...
    static unlockAccount(accountAddr, accountPwd) {
        return new Promise(resolve => {
            this.web3.eth.personal.unlockAccount(accountAddr, accountPwd, Config.ACCOUNT_UNLOCK_DURATION)
                .then(_ => resolve(true))
                .catch(error => {
                    console.log(error);
                    resolve(false);
